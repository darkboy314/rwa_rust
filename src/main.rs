...
fn main() {
    ...
    // REVB configuration (can be enabled by command line argument in future)
    let revb_mode = false; // Set to true to enable REVB variant
    ...
    // Create CSV headers
    let headers = vec![
        "E_D", "E_P", "E_V", "E_DP", "E_PV", "E_cf", "sigma_D", "sigma_P", "sigma_V", "sigma_DP", "sigma_PV", "sigma_cf",
        "m11", "m12", "m21", "m22", "q", "r", "p1", "p2", "fun"
    ];
    ...
    let e_cf = if revb_mode { 580.0 * 1.5 } else { 580.0 }; // For REVB E_cf' = 1.5 * E_cf
    let var_cf = if revb_mode { 10.0 * 4.0 } else { 10.0 }; // For REVB Var_cf' = 4 * Var_cf (2^2)
    
    // Build covariance matrix
    let mean = vec![mu_d, mu_p, mu_v];
    let cov = vec![
        vec![sigma_d * sigma_d, rho_dp * sigma_d * sigma_p, rho_dv * sigma_d * sigma_v],
        vec![rho_dp * sigma_d * sigma_p, sigma_p * sigma_p, rho_pv * sigma_p * sigma_v],
        vec![rho_dv * sigma_d * sigma_v, rho_pv * sigma_p * sigma_v, sigma_v * sigma_v],
    ];
    ...
    if let Err(e) = generate_figure_1(&price_samples, mu_p, sigma_p, &output_dir) {
        ...
        println!("Figure 1 saved to {}/figure-1/price_distribution.png", output_dir);
    }
    
    if let Err(e) = generate_figure_2(&cost_samples, e_cf, var_cf, &output_dir) {
        println!("Warning: Failed to generate Figure 2: {}", e);
    } else {
        println!("Figure 2 saved to {}/figure-2/cost_distribution.png", output_dir);
    }
...
fn process_iteration(e_cf: f64, var_cf: f64, mean: &[f64], cov: &[Vec<f64>]) -> (Vec<f64>, Vec<f64>) {
    // Generate samples
    ...
    let (m11, m12, m21, m22, q, r, p1, p2, fun) = game::start_game(
        100,
        mean_d, mean_p, mean_v, mean_dp, mean_pv,
        mean_cf, s_cf,
        s_d, s_p, s_v, s_dp, s_pv,
    );
    
    // Collect results
    let result_data = vec![
        mean_d, mean_p, mean_v, mean_dp, mean_pv, mean_cf,
        s_d, s_p, s_v, s_dp, s_pv, s_cf,
        m11, m12, m21, m22,
        q, r, p1, p2, fun,
    ];
...
fn write_results(results: &[Vec<f64>], headers: &[&str], filename: &str) -> Result<(), Box<dyn std::error::Error>> {
    use std::path::Path;
...
fn generate_figure_1(price_samples: &[f64], mu_p: f64, sigma_p: f64, output_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let max_val = price_samples.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
    
    let mut chart = ChartBuilder::on(&root)
        .caption("Figure 1 - Distribution of Price Samples and PDF", ("sans-serif", 20))
        .margin(5)
    ...
    chart.draw_series(LineSeries::new(pdf_points, &RED))?
...
fn generate_figure_2(cost_samples: &[f64], e_cf: f64, var_cf: f64, output_dir: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let max_val = cost_samples.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
    
    let mut chart = ChartBuilder::on(&root)
        .caption("Figure 2 - Distribution of Operating Cost Samples and PDF", ("sans-serif", 20))
        .margin(5)
        .x_label_area_size(30)
        .y_label_area_size(30)
    ...
    chart.draw_series(LineSeries::new(pdf_points, &BLUE))?
